"""
MEXC ECU Withdraw History Screenshot Generator

Generates withdrawal history screenshots for MEXC with ECU (Ecuadorian) currency.
Uses 4 transactions with configurable amounts, bank names, and account numbers.
ECU is converted to "USD" for display (business logic preserved).
"""
from PIL import Image
from _text_line_render.text_draw import execute as text_draw_execute


def render_mexc_ecu_withdraw_history(
    tran_1: str,
    tran_2: str,
    tran_3: str,
    tran_4: str,
    ...
    template_path: str = "templates/mexc_ecu_withdraw_history.png",
    output_path: str = "output/result.png"
) -> str:
    """
    Render MEXC ECU withdrawal history screenshot with 4 transactions.

    Args:
        tran_1: Transaction 1 amount (e.g., "100.50")
    ...
    # Currency suffix for ECU (business logic: ECU → "USD" conversion for MEXC)
    currency = " USD"
